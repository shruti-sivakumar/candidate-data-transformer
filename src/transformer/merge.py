...
def _field_family_confidence(profile: CanonicalProfile) -> dict[str, float]:
...
def _overall_confidence(profile: CanonicalProfile) -> float:
    ...
    scores = _field_family_confidence(profile)
    numerator = 0.0
    denominator = 0.0
...
def merge_records(records: list[NormalizedRecord]) -> tuple[CanonicalProfile, list[AuditEvent]]:
    ...
    profile = profile.model_copy(update={"overall_confidence": _overall_confidence(profile)})
